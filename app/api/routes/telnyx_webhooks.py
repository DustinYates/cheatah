...
@router.get("/tools/get-classes")
async def get_classes_tool() -> JSONResponse:
    """Proxy endpoint for Telnyx AI Assistant to fetch Jackrabbit class openings.

    Fetches class data from Jackrabbit's OpeningsJson API and returns a trimmed
    response with only the fields the AI agent needs, keeping the payload small
    enough for the Telnyx AI to process reliably.
    """
    import httpx
    ...
    try:
        ...
        for c in classes_raw:
            ...
            calc_openings = openings.get("calculated_openings", 0)

            # Skip classes with no openings
            if calc_openings <= 0:
                continue
        ...
        return JSONResponse(
            status_code=502,
            content={"error": f"Jackrabbit API returned {e.response.status_code}"},
        )
    except Exception as e:
        logger.error(f"[TOOL] get_classes error: {e}", exc_info=True)
        return JSONResponse(
            status_code=500,
            content={"error": str(e)},
        )
