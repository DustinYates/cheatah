...
# =============================================================================
# Test: Dynamic Variables Endpoint
# =============================================================================


class TestDynamicVariables:
    """Tests for /dynamic-variables endpoint."""

    def test_dynamic_variables_returns_prompt(self):
        """Test that dynamic variables endpoint returns a prompt."""
        with patch("app.api.routes.telnyx_webhooks.get_db") as mock_get_db:
            mock_session = AsyncMock()

            # Mock tenant voice config lookup
            mock_voice_config = MagicMock()
            mock_voice_config.tenant_id = 3
            mock_voice_config.telnyx_phone_number = "+18321234567"

            mock_result = MagicMock()
            mock_result.scalar_one_or_none.return_value = mock_voice_config
            mock_session.execute = AsyncMock(return_value=mock_result)
            mock_get_db.return_value = mock_session

            response = client.post(
                "/api/v1/telnyx/dynamic-variables",
                json={
                    "to": "+18321234567",
                    "from": "+15551234567",
            ...
            assert response.status_code == 200
            data = response.json()
            # Should return X (the prompt variable)
            assert "X" in data or "error" not in data.get("status", "")

    def test_dynamic_variables_missing_phone(self):
        """Test handling when phone numbers are missing."""
        response = client.post(
            "/api/v1/telnyx/dynamic-variables",
            json={},  # No phone numbers
        )

        assert response.status_code == 200
        # Should return fallback prompt or error message
...
# =============================================================================
# Issue #3: Spanish AI Agent - Language Detection & Prompt Tests
# =============================================================================


class TestSpanishLanguageDetection:
    """Tests for Spanish language detection via voice_phone_number routing.

    The system uses two phone numbers per tenant:
    - telnyx_phone_number: English line
    - voice_phone_number: Spanish line

    When a call comes in to voice_phone_number, the AI should:
    1. Find the tenant config (lookup must check voice_phone_number too)
    2. Detect the language as Spanish
    3. Use the Spanish prompt and voice transformation
    4. Add Spanish transfer instructions
    """

    ENGLISH_NUMBER = "+18321234567"
    SPANISH_NUMBER = "+12817679141"
    CALLER_NUMBER = "+15551234567"

    def _make_mock_config(self, has_spanish=True):
        """Create a mock TenantSmsConfig with optional Spanish line."""
        config = MagicMock()
        config.tenant_id = 3
        config.telnyx_phone_number = self.ENGLISH_NUMBER
        config.voice_phone_number = self.SPANISH_NUMBER if has_spanish else None
        config.voice_enabled = True
        return config

    def test_spanish_number_finds_tenant(self):
        """Test that calling the Spanish number finds the tenant config.

        This is the critical bug: get_dynamic_variables must look up
        voice_phone_number, not just telnyx_phone_number.
        """
        with patch("app.api.routes.telnyx_webhooks.get_db") as mock_get_db:
            mock_session = AsyncMock()
            mock_config = self._make_mock_config()

            # First query (normalized telnyx_phone_number) - no match
            # Second query (unnormalized telnyx_phone_number) - no match
            # But since we now also check voice_phone_number, it should match
            mock_result = MagicMock()
            mock_result.scalar_one_or_none.return_value = mock_config
            mock_session.execute = AsyncMock(return_value=mock_result)
            mock_get_db.return_value = mock_session

            with patch("app.api.routes.telnyx_webhooks.PromptService") as MockPromptService:
                mock_ps = MagicMock()
                mock_ps.has_dedicated_voice_prompt = AsyncMock(return_value=False)
                mock_ps.get_all_channel_prompts = AsyncMock(return_value={
                    "voice_es_prompt": "Hola, soy el asistente de natación."
                })
                mock_ps.compose_prompt_voice = AsyncMock(return_value="Hello, I'm the swim assistant.")
                MockPromptService.return_value = mock_ps

                # Mock voice config (no transfer)
                mock_voice_result = MagicMock()
                mock_voice_result.scalar_one_or_none.return_value = None

                # Set up execute to return config first, then voice config
                call_count = 0
                ...
                response = client.post(
                    "/api/v1/telnyx/dynamic-variables",
                    json={
                        "to": self.SPANISH_NUMBER,
                        "from": self.CALLER_NUMBER,
                        "call_control_id": "v3:test-spanish-call",
                    },
                )

                assert response.status_code == 200
                data = response.json()
                # Should NOT return the fallback prompt
                assert "X" in data
                # The prompt should contain Spanish content
                assert data["X"] != ""

    def test_english_number_returns_english_prompt(self):
        """Test that calling the English number returns English prompt."""
        with patch("app.api.routes.telnyx_webhooks.get_db") as mock_get_db:
            mock_session = AsyncMock()
            mock_config = self._make_mock_config()
            ...
            with patch("app.api.routes.telnyx_webhooks.PromptService") as MockPromptService:
                ...
                response = client.post(
                    "/api/v1/telnyx/dynamic-variables",
                    json={
                        "to": self.ENGLISH_NUMBER,
                        "from": self.CALLER_NUMBER,
                        "call_control_id": "v3:test-english-call",
                    },
                )
    ...
    def test_language_detection_normalized_match(self):
        """Test that phone normalization works for Spanish detection."""
        from app.api.routes.telnyx_webhooks import _normalize_phone

        # Various formats of the same Spanish number should normalize
        spanish_formats = [
            "+12817679141",
            "12817679141",
            "2817679141",
        ...
        normalized = _normalize_phone("+12817679141")
        for fmt in spanish_formats:
            assert _normalize_phone(fmt) == normalized, (
                f"Format '{fmt}' did not normalize to match '+12817679141'"
            )

    def test_spanish_prompt_selection_uses_voice_es_prompt(self):
        """Test that Spanish calls use voice_es_prompt when available."""
        with patch("app.api.routes.telnyx_webhooks.get_db") as mock_get_db:
            mock_session = AsyncMock()
            mock_config = self._make_mock_config()

            spanish_prompt = "Hola, bienvenido a clases de natación."
            english_prompt = "Hello, welcome to swim classes."
            ...
            with patch("app.api.routes.telnyx_webhooks.PromptService") as MockPromptService:
                ...
                mock_ps.has_dedicated_voice_prompt = AsyncMock(return_value=False)
                mock_ps.get_all_channel_prompts = AsyncMock(return_value={
                    "voice_es_prompt": spanish_prompt,
                })
                mock_ps.compose_prompt_voice = AsyncMock(return_value=english_prompt)
                MockPromptService.return_value = mock_ps
                ...
                response = client.post(
                    ...
                    json={
                        "to": self.SPANISH_NUMBER,
                        "from": self.CALLER_NUMBER,
                        "call_control_id": "v3:test-es-prompt",
                    },
                )
                ...
                data = response.json()
                assert "X" in data
                # Should contain the Spanish prompt, not the English one
                assert spanish_prompt in data["X"]
                assert english_prompt not in data["X"]

    def test_spanish_falls_back_to_english_when_no_es_prompt(self):
        """Test that Spanish calls fall back to English prompt if no voice_es_prompt."""
        with patch("app.api.routes.telnyx_webhooks.get_db") as mock_get_db:
            mock_session = AsyncMock()
            mock_config = self._make_mock_config()
            ...
            with patch("app.api.routes.telnyx_webhooks.PromptService") as MockPromptService:
                mock_ps = MagicMock()
                mock_ps.has_dedicated_voice_prompt = AsyncMock(return_value=False)
                mock_ps.get_all_channel_prompts = AsyncMock(return_value={})  # No Spanish prompt
                mock_ps.compose_prompt_voice = AsyncMock(return_value=english_prompt)
                MockPromptService.return_value = mock_ps
                ...
                response = client.post(
                    ...
                    json={
                        "to": self.SPANISH_NUMBER,
                        "from": self.CALLER_NUMBER,
                        "call_control_id": "v3:test-es-fallback",
                    },
                )
                ...
                data = response.json()
                assert "X" in data
                # Should still get a prompt (English fallback), not the generic fallback
                assert english_prompt in data["X"]

    def test_spanish_transfer_instructions_in_spanish(self):
        """Test that Spanish calls get transfer instructions in Spanish."""
        with patch("app.api.routes.telnyx_webhooks.get_db") as mock_get_db:
            mock_session = AsyncMock()
            mock_config = self._make_mock_config()

            mock_voice_config = MagicMock()
            mock_voice_config.handoff_mode = "live_transfer"
            mock_voice_config.live_transfer_number = "+18005551234"
            mock_voice_config.fallback_voice_prompt = None
            ...
            async def side_effect_execute(stmt):
                ...
                if call_count == 1:
                    ...
                    # Voice config lookup
                    result = MagicMock()
                    result.scalar_one_or_none.return_value = mock_voice_config
                    return result
                else:
                    result = MagicMock()
                    result.scalar_one_or_none.return_value = None
            ...
            with patch("app.api.routes.telnyx_webhooks.PromptService") as MockPromptService:
                ...
                mock_ps.has_dedicated_voice_prompt = AsyncMock(return_value=False)
                mock_ps.get_all_channel_prompts = AsyncMock(return_value={
                    "voice_es_prompt": "Hola, soy el asistente."
                })
                MockPromptService.return_value = mock_ps
                ...
                response = client.post(
                    ...
                    json={
                        "to": self.SPANISH_NUMBER,
                        "from": self.CALLER_NUMBER,
                        "call_control_id": "v3:test-es-transfer",
                    },
                )
                ...
                data = response.json()
                assert "X" in data
                # Should have Spanish transfer instructions
                assert "TRANSFERENCIA A HUMANO" in data["X"]
                assert "TRANSFER TO HUMAN" not in data["X"]

    def test_no_voice_phone_number_means_english(self):
        """Test that tenants without voice_phone_number always get English."""
        with patch("app.api.routes.telnyx_webhooks.get_db") as mock_get_db:
            mock_session = AsyncMock()
            mock_config = self._make_mock_config(has_spanish=False)
            ...
            with patch("app.api.routes.telnyx_webhooks.PromptService") as MockPromptService:
                mock_ps = MagicMock()
                mock_ps.has_dedicated_voice_prompt = AsyncMock(return_value=False)
                mock_ps.compose_prompt_voice = AsyncMock(return_value="English prompt here.")
                MockPromptService.return_value = mock_ps
                ...
                response = client.post(
                    ...
                    json={
                        "to": self.ENGLISH_NUMBER,
                        "from": self.CALLER_NUMBER,
                        "call_control_id": "v3:test-no-spanish",
                    },
                )
...
class TestSpanishVoiceTransformation:
    """Tests for Spanish voice prompt transformation."""

    def test_transform_chat_to_voice_es_wraps_prompt(self):
        """Test that Spanish voice transformation wraps the prompt correctly."""
        from app.domain.services.voice_prompt_transformer import transform_chat_to_voice_es

        chat_prompt = "Eres un asistente de natación. Ayuda a los clientes."
        result = transform_chat_to_voice_es(chat_prompt)

        # Should contain the original prompt
        assert chat_prompt in result
        # Should be longer than the original (wrapped with voice safety rules)
        assert len(result) > len(chat_prompt)

    def test_transform_chat_to_voice_es_different_from_english(self):
        """Test that Spanish transformation differs from English."""
        from app.domain.services.voice_prompt_transformer import (
            transform_chat_to_voice,
            transform_chat_to_voice_es,
        )

        prompt = "You are a swim assistant."
        english_result = transform_chat_to_voice(prompt)
        spanish_result = transform_chat_to_voice_es(prompt)

        # They should be different wrappers
        assert english_result != spanish_result
