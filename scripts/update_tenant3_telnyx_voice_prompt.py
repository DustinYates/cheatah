"""Update tenant 3 voice prompt with the comprehensive Telnyx AI Agent prompt.

This replaces the sectioned voice prompt with the complete BSS voice assistant
knowledge base designed for direct use in Telnyx AI agents.

Usage:
    DATABASE_URL=$(gcloud secrets versions access latest --secret=database-url) uv run python scripts/update_tenant3_telnyx_voice_prompt.py
"""
...
if not PROD_DATABASE_URL:
    ...
    print("Run with:")
    print('  DATABASE_URL=$(gcloud secrets versions access latest --secret=database-url) uv run python scripts/update_tenant3_telnyx_voice_prompt.py')
    sys.exit(1)
...
# Complete Telnyx AI Agent prompt - ready to paste into Telnyx
TELNYX_VOICE_PROMPT = """BRITISH SWIM SCHOOL VOICE ASSISTANT - KNOWLEDGE BASE
SYSTEM INSTRUCTIONS
...
# Voice prompt as a single section (complete prompt for Telnyx)
VOICE_PROMPT_SECTIONS = [
    {
        "section_key": "telnyx_complete_prompt",
        "scope": SectionScope.SYSTEM.value,
        "content": TELNYX_VOICE_PROMPT,
        "order": 0,
    },
]


async def update_voice_prompt():
    """Update the voice-specific prompt bundle for tenant 3 with the Telnyx AI agent prompt."""
    print("=" * 70)
    print(f"UPDATE TELNYX AI AGENT PROMPT FOR {TENANT_NAME}")
    print("=" * 70)
    print()
    ...
    async with AsyncSessionLocal() as db:
        ...
        if existing:
            print(f"\nExisting voice prompt found (ID: {existing.id})")
            print(f"   Name: {existing.name}")
            print(f"   Status: {existing.status}")
            print(f"   Sections: {len(existing.sections)}")
            response = input("\nDo you want to replace it with the new Telnyx AI agent prompt? (y/n): ")
            if response.lower() != "y":
                print("Aborted.")
                return

        # Create the new voice bundle
        print(f"\nCreating new Telnyx AI agent voice prompt bundle...")
        bundle = PromptBundle(
            tenant_id=TENANT_ID,
            name=f"{TENANT_NAME} Telnyx AI Agent Voice Prompt",
            version="2.0.0",
        ...
        # Create section (single complete prompt)
        print(f"\nAdding complete Telnyx prompt as single section...")
        for section_data in VOICE_PROMPT_SECTIONS:
            ...
            db.add(section)
            print(f"  Added: {section_data['section_key']} ({len(section_data['content'])} chars)")
        ...
        print(f"Prompt length: {len(TELNYX_VOICE_PROMPT)} characters")
        print()
        print("The next voice call to tenant 3 will use this Telnyx AI agent prompt.")
        print("=" * 70)


async def preview_prompt():
    """Preview the Telnyx prompt."""
    print(f"\n{'=' * 70}")
    print("TELNYX AI AGENT PROMPT PREVIEW (first 2000 chars)")
    print(f"{'=' * 70}")
    print(TELNYX_VOICE_PROMPT[:2000])
    print(f"\n... ({len(TELNYX_VOICE_PROMPT) - 2000} more characters)")
    print(f"\n{'=' * 70}")
    print(f"Total length: {len(TELNYX_VOICE_PROMPT)} characters")


if __name__ == "__main__":
    print("=" * 70)
    print("BSS Cypress-Spring Telnyx AI Agent Voice Prompt Update")
    print("=" * 70)
    print()
    print("This updates the tenant 3 voice prompt with the comprehensive")
    print("Telnyx AI agent prompt including:")
    print("  - Bilingual support (English/Spanish)")
    print("  - L. A. Fitness transcription-friendly naming")
    print("  - Complete Jackrabbit registration flow")
    print("  - Appointment booking flow")
    print("  - All pricing, policies, and level placement logic")
    print()

    preview = input("Would you like to preview the prompt first? (y/n): ")
    if preview.lower() == "y":
        asyncio.run(preview_prompt())

    proceed = input("\nProceed with database update? (y/n): ")
    if proceed.lower() == "y":
        asyncio.run(update_voice_prompt())
    else:
        print("Aborted.")
